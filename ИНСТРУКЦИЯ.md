...
| Команда | Описание |
|---|---|
| `/status` | Статус всех сервисов (MTProto Proxy, Prometheus, Grafana), uptime, CPU, RAM, диск, IP сервера |
| `/restart` | Перезапуск только MTProto Proxy |
| `/restartall` | Перезапуск всех сервисов (прокси + Prometheus + Grafana) |
| `/logs` | Последние 15 строк логов прокси |
| `/traffic` | Активные соединения, соединения с Telegram DC, трафик (получено/отправлено/всего в MB), replay-атаки |
| `/ip` | IP сервера и готовая ссылка прокси для копирования |
