...
Бот для управления прокси и автоматических уведомлений.

### Возможности

| Команда | Описание |
|---|---|
| `/status` | Статус всех сервисов, CPU, RAM |
| `/restart` | Перезапуск прокси |
| `/restartall` | Перезапуск всех сервисов |
| `/logs` | Последние логи прокси |
| `/traffic` | Статистика трафика и соединений |
| `/ip` | IP сервера и ссылка прокси |

**Автоматические уведомления:**
- Прокси упал → мгновенное уведомление
- Прокси восстановился → уведомление
- CPU > 80% или RAM > 90% → предупреждение
...
2. Вставить токен
3. Отправить сообщение боту для получения Chat ID
...
journalctl -u mtproto-monitor-bot -f    # Логи
```
